...
//				maxResult : data.maxResult,
	...
	function sort(event) {
		...
		var url = _prop.getApiMap( "speechLog.searchByCompany" );
...
//
		...
		if ( !_util.isEmpty( response.searchResultList ) ) {
			...
			$.each( response.searchResultList, function ( i, val ) {
				...
				let userName = log.userName; // 利用者名
				...
				let $stat = $( "<td></td>" )
							.addClass( "ccs-speechlog-management-listUserName" )
							.text( userName ).appendTo( $tr ); // 利用者名
	...
	function _prepareDownload( speechLogId ) {
		...
		$( dialog ).on( "hidden.bs.modal", function () {
			...
			var url = _prop.getApiMap( "speechLog.generateVoiceByCompany" );
			var json = JSON.stringify( form );
	...
	function _download(  ) {
		...
		var url = _prop.getApiMap( "speechLog.downloadFileByCompany" );
	...
	function _delOk( event ) {
		...
		var speechLog = data.speechLog;
		var id = speechLog.speechLogId;
		...
		// API 送信
		var url = _prop.getApiMap( "speechLog.del" );
